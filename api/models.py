...
class Token(models.Model):
    ...
    token_number = models.IntegerField(blank=True)
    ...
    def save(self, *args, **kwargs):
        if self.doctor and not self.clinic: self.clinic = self.doctor.clinic
        if not self.pk:
            last_token = Token.objects.filter(clinic=self.clinic, date=self.date).order_by('-token_number').first()
            self.token_number = (last_token.token_number + 1) if last_token else 1
        if self.status == 'completed' and self.completed_at is None: self.completed_at = timezone.now()
...
class Consultation(models.Model):
    ...
    # The old, simple prescription field is now removed.

    def __str__(self):
        return f"Consultation for {self.patient.name} on {self.date.strftime('%Y-%m-%d')}"

# This is your brilliant new "Smart Prescription" model
class PrescriptionItem(models.Model):
    consultation = models.ForeignKey(Consultation, related_name='prescription_items', on_delete=models.CASCADE)
    medicine_name = models.CharField(max_length=200)
    dosage = models.CharField(max_length=100) # e.g., "1 tablet", "10 ml"
    duration_days = models.IntegerField()
    
    # Timing fields for automated reminders
